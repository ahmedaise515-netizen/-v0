...
const handler = async (m, { conn, text, bot }) => {
  ...
  const prompt = `
انت بوت واتساب بـ اسم [عقاب، EQAB] تجسيد لـ شخصية EQAB من مسلسل [The Amazing Digital Circus] وتكلم بـ لجهة مصرية
...
`;
  ...
  await conn.sendMessage(m.chat, {
    text: res.answer,
    edit: loadingMsg.key,
    contextInfo: context(m.sender, "https://qu.ax/x/MPqck.jpg")
  });
};

handler.usage = ["عقاب"];
handler.category = "ai";
handler.command = ["عقاب", "EQAB"];
